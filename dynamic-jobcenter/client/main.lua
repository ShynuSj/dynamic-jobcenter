CreateThread(function()
    ...
    RegisterNUICallback("getjob", function(data)
        TriggerServerEvent("dynamic-jobcenter:server:setjob", data.jobindex, closestidx)
    end)
    ...
    while true do
        if closestpos ~= nil and closestdist < 2.0 and isopen == false then
            ...
            if closestdist < 1.0 and IsControlJustReleased(0,38) then
               -- if ESX.GetPlayerData().job.name == 'cm' or ESX.GetPlayerData().job.name == 'masz' or ESX.GetPlayerData().job.name == 'act' or ESX.GetPlayerData().job.name == 'nbs' or ESX.GetPlayerData().job.name == 'gk' or ESX.GetPlayerData().job.name == 'rgk' then
                --ESX.ShowNotification("You cannot take legal jobs since you are in a Gang")
                     SendNUIMessage({
                    action = "open",
